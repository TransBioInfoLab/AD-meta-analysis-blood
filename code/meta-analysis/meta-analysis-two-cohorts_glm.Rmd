...
readr::write_csv(
  ...
  file = paste0(dir.result.meta.analysis.mci, "meta_analysis_glm_fixed_effect_ADNI_and_AIBL_MCI_vs_CN_single_cpg.csv")
)
...
readr::write_csv(
  ...
  file = paste0(dir.result.meta.analysis.mci,"meta_analysis_glm_fixed_effect_ADNI_and_AIBL_MCI_vs_CN_single_cpg_annotated.csv")
)
...
readr::write_csv(
  ...
  file = paste0(
    ...
    "meta_analysis_glm__fixed_effect_ADNI_and_AIBL_MCI_vs_CN_single_cpg_annotated_pvalue_cut_off_0_05.csv"
  )
)
```
...
```{R}
meta_df_MCI_vs_CN.annotated.sig <- readr::read_csv(
  file = paste0(
    dir.result.meta.analysis.mci,
    "meta_analysis_glm__fixed_effect_ADNI_and_AIBL_MCI_vs_CN_single_cpg_annotated_pvalue_cut_off_0_05.csv"
  ),
...
dim(meta_df_MCI_vs_CN.annotated.sig)
table(meta_df_MCI_vs_CN.annotated.sig$pVal.final.bacon < 1E-5) 
table(meta_df_MCI_vs_CN.annotated.sig$fdr.bacon < 0.05)
ggpubr::gghistogram(meta_df_MCI_vs_CN.annotated.sig$fdr.bacon)
```

### pVal.final.bacon < 1E-5
```{R}
meta_df_MCI_vs_CN.annotated.sig %>% dplyr::filter(pVal.final.bacon < 1E-5) %>% gt::gt()
```
