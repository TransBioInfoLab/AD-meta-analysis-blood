...
multi_cohorts_MCI_vs_CN <- Reduce(
  function(x,y, ...) inner_join(
    x[c("inputRegion_chr",grep("ADNI_|AIBL_",colnames(x),value = TRUE))], 
    y[c("inputRegion_chr",grep("ADNI_|AIBL_",colnames(y),value = TRUE))], 
    ..., 
  ...
  list.of.results[grep("MCI_vs_CN",names(list.of.results))]
) %>% unique()# keep only results with the two cohorts

lapply(list.of.results,dim)
dim(multi_cohorts_MCI_vs_CN)
...
```{R meta_mci, eval = FALSE}
meta_df_MCI_vs_CN <- multi_cohorts_MCI_vs_CN %>% calculate_meta_analysis()
meta_df_MCI_vs_CN_fixed <- meta_df_MCI_vs_CN %>% calculate_meta_analysis_pvalue(pvalue.type = "fixed")

readr::write_csv(
  meta_df_MCI_vs_CN_fixed  %>% as.data.frame(),
  file = paste0(
    dir.result.meta.analysis.mci, "meta_analysis_glm_fixed_effect_ADNI_and_AIBL_MCI_vs_CN_DMR.csv")
)
```
...
readr::write_csv(
  meta_df_AD_vs_CN.annotated.comethylated  %>% as.data.frame(),
  file = paste0(dir.result.meta.analysis.mci, "meta_analysis_glm_fixed_effect_ADNI_and_AIBL_MCI_vs_CN_DMR_annotated_and_comethylated.csv")
)
...
```{R, eval = FALSE}
meta_df_MCI_vs_CN.annotated <- meta_df_MCI_vs_CN_fixed %>% add_annotation()
meta_df_MCI_vs_CN.annotated.sig <- meta_df_MCI_vs_CN.annotated %>% dplyr::filter(pVal.final.bacon < 0.05) 

readr::write_csv(
  meta_df_MCI_vs_CN.annotated  %>% as.data.frame(),
  file = paste0(dir.result.meta.analysis.mci, "meta_analysis_glm_fixed_effect_ADNI_and_AIBL_MCI_vs_CN_DMR_annotated.csv")
)

readr::write_csv(
  meta_df_MCI_vs_CN.annotated.sig  %>% as.data.frame(),
  file = paste0(
    dir.result.meta.analysis.mci,
    "meta_analysis_glm_fixed_effect_ADNI_and_AIBL_MCI_vs_CN_DMR_annotated_pvalue_cut_off_0_05.csv"
  )
)
meta_df_MCI_vs_CN.annotated.comethylated <- meta_df_MCI_vs_CN.annotated[meta_df_MCI_vs_CN.annotated$AIBL_nCoMethRegions_num > 0 | meta_df_MCI_vs_CN.annotated$ADNI_nCoMethRegions_num > 0,]
meta_df_MCI_vs_CN.annotated.comethylated$fdr.bacon <- p.adjust(meta_df_MCI_vs_CN.annotated.comethylated$pVal.final.bacon,method = "BH")

readr::write_csv(
  meta_df_MCI_vs_CN.annotated.comethylated  %>% as.data.frame(),
  file = paste0(dir.result.meta.analysis.mci, "meta_analysis_glm_fixed_effect_ADNI_and_AIBL_MCI_vs_CN_DMR_annotated_and_comethylated.csv")
)
