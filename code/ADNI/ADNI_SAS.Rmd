...
dir.create("~/Dropbox (BBSR)/AD-meta-analysis-blood-samples/LW/ADNI_GLMM/DATA_for_SAS_models_MCI_CN//")
for(start in seq(1,nrow(adni.se),by = 1000)){
  print(start)
  end <- start + 1000 - 1
  idx <- which(start == seq(1,nrow(adni.se),by = 1000))
  if(end > nrow(adni.se)) end <- nrow(adni.se)
  adni.se.selected <- adni.se[start:end,adni.se$DX %in% c("MCI","CN")]
  final <- str_data(adni.se.selected)
  final$DIAGNOSIS_merged <- final$VISCODE <- final$visit <- NULL
  readr::write_csv(
    final,
...
```{R, eval = FALSE}
files <- dir(
  file.path(dir.base,"LW/ADNI_GLMM/MCI_CN/Results/"),
  full.names = T,
  recursive = T,
  pattern = "_[0-9]*.csv"
)
...
readr::write_csv(
  glmm, 
  file = file.path(dir.results.glm,"ADNI_MCI_vs_CN_glmm_using_beta_SAS.cvs")
)
```
...
logistic_mixed_model_mci_cn.annnotated <- logistic_mixed_model_mci_cn %>% annotate_results(adni.se)

readr::write_csv(
  x = logistic_mixed_model_mci_cn.annnotated,
  file = file.path(dir.results.glm,"ADNI_MCI_vs_CN_glmm_using_beta_all_annotated.csv")
)
...
logistic_mixed_model_mci_cn.annnotated <- readr::read_csv(
  file.path(dir.results.glm,"ADNI_MCI_vs_CN_glmm_using_beta_all_annotated.csv")
)
...
```{R}
logistic_mixed_model_mci_cn.with.inflation <- estimation_of_inflation(data = logistic_mixed_model_mci_cn.annnotated)
...
readr::write_csv(
  ...
  file = file.path(dir.results.glm,"ADNI_MCI_vs_CN_glmm_using_beta_with_inflation.csv")
)
```

```{R}
plot_inflation(logistic_mixed_model_mci_cn.with.inflation, "ADNI MCI vs CN - single cpg -  Logistic regression model")

qqPlot(
  logistic_mixed_model_mci_cn.with.inflation$data.with.inflation$pValue.bacon, ci = TRUE, 
  main = paste0("AIBL MCI vs CN - single cpg - logistic regression mode - pValue.bacon")
) 
