...
dir.result.meta.analysis <- file.path(dir.results, "meta_analysis/DMR_ordinal_logistic_regression_model/")
...
list.of.results <- plyr::alply(results.files,.margins = 1,.fun = function(f){
  ...
  print(colnames(data))
  dataset <- stringr::str_extract(basename(f),pattern = "AIBL|ADNI")
  ...
  data <- data %>% rename_with(
    ...
    contains(c("nCoMethRegions_num","bacon","fdr","tValue","z.value","Pr...z..","Std..Error","t.value","Error","Estimate","Pr...t.."))
  )
  colnames(data)[1] <-  paste0(dataset,"_",  colnames(data)[1])
  data  
})
names(list.of.results) <- stringr::str_extract(basename(results.files),pattern = "AIBL|ADNI")
...
meta_df_fixed <- meta_df %>% calculate_meta_analysis_pvalue(pvalue.type = "fixed")

readr::write_csv(
  meta_df_fixed  %>% as.data.frame(),
  file = paste0(dir.result.meta.analysis, "meta_analysis_olrm_fixed_effect_ADNI_and_AIBL_DMR.csv")
)
```
...
```{R, eval = FALSE}
meta_df.annotated <- meta_df_fixed %>% add_annotation()
meta_df.annotated.sig <- meta_df.annotated %>% dplyr::filter(pVal.final.bacon < 0.05) 

readr::write_csv(
  meta_df.annotated  %>% as.data.frame(),
  file = paste0(dir.result.meta.analysis, "meta_analysis_orlm_fixed_effect_ADNI_and_AIBL_DMR_annotated.csv")
)

readr::write_csv(
  meta_df.annotated.sig  %>% as.data.frame(),
  file = paste0(
    dir.result.meta.analysis,
    "meta_analysis_orlm_fixed_effect_ADNI_and_AIBL_DMR_annotated_pvalue_cut_off_0_05.csv"
  )
)

meta_df.annotated.comethylated <- meta_df.annotated[meta_df.annotated$AIBL_nCoMethRegions_num > 0 | meta_df.annotated$ADNI_nCoMethRegions_num > 0,]
meta_df.annotated.comethylated$fdr.bacon <- p.adjust(meta_df.annotated.comethylated$pVal.final.bacon,method = "BH")

readr::write_csv(
  meta_df.annotated.comethylated  %>% as.data.frame(),
  file = paste0(dir.result.meta.analysis, "meta_analysis_orlm_fixed_effect_ADNI_and_AIBL_DMR_annotated_and_comethylated.csv")
)
